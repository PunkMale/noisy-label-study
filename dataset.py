...
class MyCIFAR10(datasets.CIFAR10):
    ...
    def __getitem__(self, index: int):
        img, target, ground_truth = self.data[index], self.targets[index], self.ground_truth[index]
        ...
        return img, target, ground_truth
...
def cifar10(root, noise_type, noise_rate, model_name, tuning=False):
    if noise_type == 'sym':
    ...
    transform = transforms.Compose([
        ...
        transforms.Resize((227, 227)),  # 调整图像大小为AlexNet所需的尺寸
    ...
    if tuning:
        ...
        test_dataset = MyCIFAR10(root=root,
                                 train=True,
                                 transform=eval_transform)
        _, test_dataset = random_split(test_dataset, [num_train, num_eval],
                                       generator=torch.Generator().manual_seed(42))
...
def cifar100(root, noise_type, noise_rate, tuning=False):
    ...
    if tuning:
        ...
        test_dataset = MyCIFAR100(root=root,
                                  train=True,
                                  transform=eval_transform)
        _, test_dataset = random_split(test_dataset, [num_train, num_eval],
                                       generator=torch.Generator().manual_seed(42))
